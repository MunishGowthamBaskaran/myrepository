/*
 ...
 Based on work by Johann Burkhard & Marcel StÃ¶r
...
(function($) {
	...
	var methods = {
		...
		highlight : function(searchString) {
			...
			//If lenient settings then we replace the search string with a regex character class A->[AaÃ„Ã¤Ã¥....]
			if(settings.lenient) {
	...
	function setHighlight(node, pattern) {
		...
		if(node.nodeType == 3) {
			var changed = 0;
			var replacement = node.nodeValue	.replace(pattern, function(match) {
				changed = 1;
				//return "<span class='highlight'>" + match + "</span>";
			});
			if(changed) {
				node.parentNode.style.backgroundColor = "#ff6";
				skip = 1;
			}
		} else {
